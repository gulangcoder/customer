...
public class OrganizationController {
    ...
    public ResponseEntity getOrgList(){
        ...
        try {
            ...
            return ResponseEntity.ok().body(ResponseLogMessageHandel.initSuccessResponseData(orgList));
        }catch (Exception e){
    ...
    public ResponseEntity getListByOrgId(@RequestParam(value = "orgId",required = false) String orgId,
        ...
        return ResponseEntity.ok().body(ResponseLogMessageHandel.initSuccessResponseData(maps));
    }
    ...
    public ResponseEntity saveOrg(@RequestBody  Organization org){
        if(StringUtil.isEmpty(org.getParentOrgId())){
            return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert(ENTITY_NAME, "The user have logined", "上级组织不可为空")).body(null);
        }
        UserInfo userInfo = UserContextUtil.getUserInfo();
    ...
    public ResponseEntity getOrgById(@RequestParam("orgId")  String orgId){
        try{
            return ResponseEntity.ok().body(ResponseLogMessageHandel.initSuccessResponseData(organizationService.getOrgById(orgId)));
        }catch (Exception e){
           String localeTipMsg = LocaleMessage.get("message.query.errorMessage");
    ...
    public ResponseEntity updateOrgStatus(@RequestBody Map data){
        String localeTipMsg=LocaleMessage.get("message.system.request.param.exception");
